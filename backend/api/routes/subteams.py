...
from api.request_model.subteam_request_schema import CreateSubteamRequest, DeleteSubteamRequest, AssignTaskToSubteamRequest
...
async def create_subteam(request : CreateSubteamRequest):
    ...
    return {"message": "Subteam created successfully", "data": {created_subteam}}
...
async def remove_task_from_subteam(request: AssignTaskToSubteamRequest):
    # Check if subteam exists
    if not subteams_collection.find_one({"team_name": request.team_name}):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
